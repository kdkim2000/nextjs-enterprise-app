...
export default function UserAutocomplete({
    ...
    if (value && !selectedUser) {
      const fetchUserById = async () => {
        try {
          ...
          const response = await adminApi.get(`/users?id=${value}&page=1&limit=1`);
  ...
  const searchUsers = useCallback(
    debounce(async (searchTerm: string) => {
      ...
      setLoading(true);
      try {
        const response = await adminApi.get(`/users?username=${searchTerm}&name=${searchTerm}&email=${searchTerm}&page=1&limit=50`);
