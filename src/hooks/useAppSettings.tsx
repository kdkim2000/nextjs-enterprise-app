...
export function AppSettingsProvider({ children }: { children: ReactNode }) {
  ...
  const fetchSettings = useCallback(async () => {
    try {
      ...
      // Merge with defaults (settings from API override defaults)
      setSettings({
   ...
   */
  const getLocalizedSetting = useCallback((baseKey: string, defaultValue?: string): string => {
...
export function useAppSettings() {
  ...
  if (!context) {
    ...
    return {
      ...
      loading: false,
      error: null,
      getSetting: <K extends keyof AppSettings>(key: K, defaultValue?: AppSettings[K]) =>
        defaultValue !== undefined ? defaultValue : DEFAULT_APP_SETTINGS[key] as AppSettings[K],
      getLocalizedSetting: (baseKey: string, defaultValue?: string) => defaultValue || '',
 ...
 */
export function useLocalizedAppSetting(baseKey: string, defaultValue?: string): string {
