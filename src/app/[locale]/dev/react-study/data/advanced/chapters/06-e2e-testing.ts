...
test.describe('User Management', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await loginPage.login('admin@test.com', '<TEST_PASSWORD>');
...
test.describe('Authentication', () => {
  test('should login with valid credentials', async ({ page }) => {
    ...
    await loginPage.login('user@test.com', '<TEST_PASSWORD>');
  ...
  test('should remember login state', async ({ page }) => {
    ...
    // "로그인 상태 유지" 체크 후 로그인
    await loginPage.loginWithRemember('user@test.com', '<TEST_PASSWORD>');
    await expect(page).toHaveURL('/dashboard');
