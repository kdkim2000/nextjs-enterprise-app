...
export default function PostDetailPage() {
  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        setLoading(true);
        const response = await contentApiClient.get(`/posts/${postId}`);
        if (response.success) {
  ...
  useEffect(() => {
    const fetchComments = async () => {
      try {
        const response = await contentApiClient.get(`/comments/post/${postId}`);
        if (response.success) {
  ...
  useEffect(() => {
    const fetchAttachments = async () => {
      try {
        const response = await commonApiClient.get(`/attachments/post/${postId}`);
        if (response.success) {
  ...
  const handleLike = async () => {
    try {
      const response = liked
        ? await contentApiClient.post(`/posts/${postId}/unlike`)
        : await contentApiClient.post(`/posts/${postId}/like`);
  ...
  const handleSubmitComment = async () => {
    ...
    try {
      ...
      const response = await contentApiClient.post('/comments', {
        postId,
        content: newComment
      });
      ...
      if (response.success) {
        setNewComment('');
        // Refetch comments
        const commentsResponse = await contentApiClient.get(`/comments/post/${postId}`);
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await contentApiClient.delete(`/posts/${postId}`);
      if (response.success) {
  ...
  if (error || !post) {
    return (
      <Container maxWidth="lg" sx={{ py: 4 }}>
        ...
        <Button onClick={() => router.push(`/admin/boards/${boardTypeId}`)} sx={{ mt: 2 }}>
          Back to Board
        </Button>
      </Container>
    );
    ...
    <Container maxWidth="lg" sx={{ py: 4 }}>
      ...
      <Breadcrumbs sx={{ mb: 2 }}>
        ...
        <Link
          ...
          color="inherit"
          sx={{ cursor: 'pointer' }}
          onClick={() => router.push(`/admin/boards/${boardTypeId}`)}
        >
      ...
      <Box sx={{ mb: 2, display: 'flex', justifyContent: 'space-between' }}>
        <Button startIcon={<ArrowBack />} onClick={() => router.push(`/admin/boards/${boardTypeId}`)}>
          Back to List
        </Button>
        {isAuthor && (
