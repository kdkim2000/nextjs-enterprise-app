...
export const useBoardTypeManagement = () => {
  ...
  const fetchBoardTypes = useCallback(async () => {
    try {
      ...
      const response = await contentApiClient.get(`/board-types?${queryString}`);
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      const response = isNew
        ? await contentApiClient.post('/board-types', payload)
        : await contentApiClient.put(`/board-types/${editingBoardType.id}`, payload);
  ...
  const handleDeleteConfirm = useCallback(async () => {
    try {
      ...
      for (const id of selectedForDelete) {
        const response = await contentApiClient.delete(`/board-types/${id}`);
        if (!response.success) {
  ...
  const handleViewStats = useCallback(async (id: string | number) => {
    ...
    try {
      const response = await contentApiClient.get(`/board-types/${id}/stats`);
      if (response.success) {
