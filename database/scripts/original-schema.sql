...
CREATE TABLE public.mail_messages (
    ...
    created_at timestamp with time zone DEFAULT now(),
    updated_at timestamp with time zone DEFAULT now(),
    CONSTRAINT valid_external_status CHECK (((external_status IS NULL) OR ((external_status)::text = ANY ((ARRAY['pending'::character varying, 'sent'::character varying, 'failed'::character varying])::text[]))))
);
...
CREATE TABLE public.mail_recipients (
    ...
    created_at timestamp with time zone DEFAULT now(),
    CONSTRAINT valid_recipient_type CHECK (((recipient_type)::text = ANY ((ARRAY['to'::character varying, 'cc'::character varying, 'bcc'::character varying])::text[])))
);
...
CREATE TABLE public.mail_user_messages (
    ...
    created_at timestamp with time zone DEFAULT now(),
    updated_at timestamp with time zone DEFAULT now(),
    CONSTRAINT valid_folder CHECK (((folder)::text = ANY ((ARRAY['inbox'::character varying, 'sent'::character varying, 'draft'::character varying, 'trash'::character varying])::text[]))),
    CONSTRAINT valid_role CHECK (((role)::text = ANY ((ARRAY['sender'::character varying, 'to'::character varying, 'cc'::character varying, 'bcc'::character varying])::text[])))
);
...
-- PostgreSQL database dump complete (simplified - no triggers/FK)
