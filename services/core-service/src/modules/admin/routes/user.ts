...
const router = Router();
const logger = getLogger('core-service:admin:user-routes');
  ...
  try {
    ...
    if (!userId) {
      return res.status(401).json({
        success: false,
        error: { code: 'AUTH_002', message: 'Invalid access token - userId missing' }
      });
    }
    ...
    if (!user) {
      return res.status(404).json({
        success: false,
        error: { code: 'RES_101', message: 'User not found' }
      });
    }
    ...
    res.json({ success: true, preferences: apiPreferences });
  } catch (error: any) {
...
router.put('/preferences', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    const { language, theme, rowsPerPage, emailNotifications, systemNotifications, sessionTimeout, favoriteMenus, recentMenus } = req.body;
...
router.get('/favorite-menus', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    if (!favoriteMenuIds.length) return res.json({ menus: [] });

    const allMenus = await menuService.getAllMenus({});
    const favoriteMenus = allMenus.filter(m => favoriteMenuIds.includes(m.id)).map(transformMenuToAPI);
...
router.post('/favorite-menus', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    if (!favoriteMenus.includes(menuId)) {
      ...
      await preferencesService.createUserPreferences({ userId, favoriteMenus });
    }
...
router.get('/recent-menus', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    if (!recentMenuIds.length) return res.json({ menus: [] });

    const allMenus = await menuService.getAllMenus({});
    const recentMenus = allMenus.filter(m => recentMenuIds.includes(m.id)).map(transformMenuToAPI);
...
router.put('/profile', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    const safeUser = { ...rest, mfaEnabled: mfa_enabled, ssoEnabled: sso_enabled, avatarUrl: dbAvatarUrl, avatar_image: dbAvatarImage, lastLogin: last_login, createdAt: created_at, updatedAt: updated_at };
...
router.get('/', authenticateToken, async (req: Request, res: Response) => {
  try {
    const { search, loginid, name_ko, name_en, email, employee_number, phone_number, mobile_number, user_category, position, role, status, page = '1', limit = '50' } = req.query;
    ...
    const users = await userService.getAllUsers({
      search: search as string, loginid: loginid as string, name_ko: name_ko as string, name_en: name_en as string,
      email: email as string, employee_number: employee_number as string, phone_number: phone_number as string,
      mobile_number: mobile_number as string, user_category: user_category as string, position: position as string,
      role: role as string, status: status as string, departments, limit: limitNum, offset
    });

    const totalCount = await userService.getUserCount({ search: search as string, status: status as string, role: role as string, departments });
    ...
    const safeUsers = users.map(({ password, mfa_enabled, sso_enabled, avatar_url, avatar_image, last_login, created_at, updated_at, ...rest }) => ({
      ...rest, mfaEnabled: mfa_enabled, ssoEnabled: sso_enabled, avatarUrl: avatar_url, avatar_image, lastLogin: last_login, createdAt: created_at, updatedAt: updated_at
    }));

    res.json({ users: safeUsers, pagination: { page: pageNum, limit: limitNum, totalCount, totalPages, hasMore: pageNum < totalPages } });
  } catch (error: any) {
...
router.get('/all', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    const simpleUsers = users.map(({ id, loginid, name_ko, name_en }) => ({ id, username: loginid, name: name_ko || name_en || loginid }));
...
router.get('/:id', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    const safeUser = { ...rest, mfaEnabled: mfa_enabled, ssoEnabled: sso_enabled, avatarUrl: avatar_url, avatar_image, lastLogin: last_login, createdAt: created_at, updatedAt: updated_at };
    res.json({ user: safeUser });
  } catch (error: any) {
...
router.post('/', authenticateToken, requireAdmin, async (req: Request, res: Response) => {
  try {
    const { loginid, password, name_ko, name_en, email, role, department, status, avatar_url, avatar_image, employee_number, phone_number, mobile_number, user_category, position } = req.body;

    if (!loginid || !password || !name_ko || !email) {
      return res.status(400).json({ error: 'Missing required fields' });
    }

    if (await userService.loginidExists(loginid)) return res.status(400).json({ error: 'Login ID already exists' });
    if (await userService.emailExists(email)) return res.status(400).json({ error: 'Email already exists' });

    const hashedPassword = await bcrypt.hash(password, 10);

    const newUser = await userService.createUser({
      id: uuidv4(), loginid, password: hashedPassword, name_ko, name_en: name_en || '', email,
      employee_number: employee_number || '', phone_number: phone_number || '', mobile_number: mobile_number || '',
      user_category: user_category || 'regular', position: position || '', department: department || '',
      status: status || 'active', avatar_url, avatar_image
    });

    const { password: _, mfa_enabled, sso_enabled, avatar_url: dbAvatarUrl, avatar_image: dbAvatarImage, ...rest } = newUser;
    const safeUser = { ...rest, mfaEnabled: mfa_enabled, ssoEnabled: sso_enabled, avatarUrl: dbAvatarUrl, avatar_image: dbAvatarImage };
    ...
    if (error.code === '23505') {
      ...
      if (detail.includes('loginid')) return res.status(400).json({ error: 'Login ID already exists' });
...
router.put('/:id', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    if (!isSelf && !isAdmin) return res.status(403).json({ error: 'Update permission required' });
...
router.delete('/:id', authenticateToken, requireAdmin, async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    if (req.user?.userId === id) return res.status(400).json({ error: 'Cannot delete your own account' });
...
router.post('/:id/reset-password', authenticateToken, requireAdmin, async (req: Request, res: Response) => {
  try {
    ...
    if (req.user?.userId === id) return res.status(400).json({ error: 'Cannot reset your own password. Use change-password endpoint instead.' });
    ...
    res.json({ message: 'Password reset successfully', user: { id: user.id, loginid: user.loginid, email: user.email } });
  } catch (error: any) {
