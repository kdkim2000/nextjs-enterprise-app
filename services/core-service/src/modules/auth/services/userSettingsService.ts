/**
 * User Settings Service - Core Service
...
function dbRowToPrefs(row: any): any {
  ...
  return {
    ...
    preferences: {
      general: {
        language: row.language || DEFAULT_SETTINGS.general.language,
        timezone: DEFAULT_SETTINGS.general.timezone,
        dateFormat: DEFAULT_SETTINGS.general.dateFormat,
        timeFormat: DEFAULT_SETTINGS.general.timeFormat
      },
      appearance: {
        theme: row.theme || DEFAULT_SETTINGS.appearance.theme,
        fontSize: DEFAULT_SETTINGS.appearance.fontSize,
        compactMode: DEFAULT_SETTINGS.appearance.compactMode,
        sidebarCollapsed: DEFAULT_SETTINGS.appearance.sidebarCollapsed
      },
      notifications: {
        email: row.email_notifications ?? DEFAULT_SETTINGS.notifications.email,
        push: DEFAULT_SETTINGS.notifications.push,
        desktop: DEFAULT_SETTINGS.notifications.desktop,
        sound: row.system_notifications ?? DEFAULT_SETTINGS.notifications.sound
      },
      dataGrid: {
        defaultPageSize: row.rows_per_page || DEFAULT_SETTINGS.dataGrid.defaultPageSize,
        showDensitySelector: DEFAULT_SETTINGS.dataGrid.showDensitySelector,
        showColumnSelector: DEFAULT_SETTINGS.dataGrid.showColumnSelector,
        showFilterPanel: DEFAULT_SETTINGS.dataGrid.showFilterPanel,
        autoRefresh: DEFAULT_SETTINGS.dataGrid.autoRefresh,
        autoRefreshInterval: DEFAULT_SETTINGS.dataGrid.autoRefreshInterval
      },
...
export async function createUserPreferences(data: { userId: string; preferences: any }): Promise<any> {
  ...
  const queryText = `
    INSERT INTO user_preferences (user_id, language, theme, rows_per_page, email_notifications, system_notifications, created_at, updated_at)
    VALUES ($1, $2, $3, $4, $5, $6, NOW(), NOW())
    ON CONFLICT (user_id) DO UPDATE SET
      language = COALESCE($2, user_preferences.language),
      theme = COALESCE($3, user_preferences.theme),
      rows_per_page = COALESCE($4, user_preferences.rows_per_page),
      email_notifications = COALESCE($5, user_preferences.email_notifications),
      system_notifications = COALESCE($6, user_preferences.system_notifications),
      updated_at = NOW()
    RETURNING *
  `;
  const result = await query(queryText, [
    userId,
    preferences.general?.language || null,
    preferences.appearance?.theme || null,
    preferences.dataGrid?.defaultPageSize || null,
    preferences.notifications?.email ?? null,
    preferences.notifications?.sound ?? null
  ]);
  return dbRowToPrefs(result.rows[0]);
}
...
export async function updateUserPreferences(userId: string, updates: { preferences: any }): Promise<any | null> {
  ...
  if (prefs.general?.language !== undefined) {
    setClauses.push('language = $' + i++);
    values.push(prefs.general.language);
  }
  if (prefs.appearance?.theme !== undefined) {
    setClauses.push('theme = $' + i++);
    values.push(prefs.appearance.theme);
  }
  if (prefs.dataGrid?.defaultPageSize !== undefined) {
    setClauses.push('rows_per_page = $' + i++);
    values.push(prefs.dataGrid.defaultPageSize);
  }
  if (prefs.notifications?.email !== undefined) {
    setClauses.push('email_notifications = $' + i++);
    values.push(prefs.notifications.email);
  }
  if (prefs.notifications?.sound !== undefined) {
    setClauses.push('system_notifications = $' + i++);
    values.push(prefs.notifications.sound);
  }
