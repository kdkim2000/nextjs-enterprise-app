...
const router = Router();
const logger = getLogger('common-service:routes:code');

/**
 * GET /codes - Get all codes with pagination
 */
router.get('/', authenticateToken, async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const transformedCodes = codes.map(c =>
      transformMultiLangFields(c, ['name', 'description'])
    );
...
/**
 * GET /codes/types/list - Get distinct code types
 */
...
/**
 * GET /codes/type/:codeType - Get codes by type
 */
...
/**
 * DELETE /codes - Bulk delete codes
 */
