...
): Promise<PaginationResult<Conversation & { tags?: any[] }>> {
  const {
    page = 1,
    limit = 20,
    category,
...
export async function getConversationById(
  ...
  const conversationQuery = `
    ...
    WHERE c.id = $1
  `;
  const conversationResult = await query(conversationQuery, [id]);
  ...
  `;
  const codeChangesResult = await query(codeChangesQuery, [id]);
...
export async function getStats(): Promise<ConversationStats> {
  ...
  const difficultyStatsQuery = `
    ...
    ORDER BY
      ...
      END
  `;
  const difficultyStats = await query(difficultyStatsQuery);
  ...
  const branchStatsQuery = `
    ...
    LIMIT 10
  `;
  const branchStats = await query(branchStatsQuery);
  ...
  return {
    ...
    byCategory: categoryStats.rows.reduce((acc: Record<string, number>, row: any) => {
      acc[row.category] = parseInt(row.count, 10);
      return acc;
    }, {}),
    byDifficulty: difficultyStats.rows.reduce((acc: Record<string, number>, row: any) => {
      acc[row.difficulty_level] = parseInt(row.count, 10);
      return acc;
    }, {}),
    byMonth: monthlyStats.rows.reduce((acc: Record<string, number>, row: any) => {
      acc[row.month] = parseInt(row.count, 10);
      return acc;
    }, {}),
    byBranch: branchStats.rows.reduce((acc: Record<string, number>, row: any) => {
...
export async function search(
  ...
  const [dataResult, countResult] = await Promise.all([
    query(dataQuery, [searchQuery, searchPattern, limit, offset]),
    query(countQuery, [searchQuery, searchPattern])
  ]);
...
export async function getFilterOptions(): Promise<{
  ...
  const [categories, difficulties, branches] = await Promise.all([
    ...
    query(difficultiesQuery),
    query(branchesQuery)
  ]);

  return {
    categories: categories.rows.map((r: any) => r.category),
    difficulties: difficulties.rows.map((r: any) => r.difficulty_level),
    branches: branches.rows.map((r: any) => r.branch_name)
  };
}
...
export async function deleteConversations(ids: string[]): Promise<{
  ...
  try {
    ...
    const existingIds = checkResult.rows.map((r: any) => r.id);
