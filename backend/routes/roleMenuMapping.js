...
// Helper function to enrich mapping with role and menu details
async function enrichMappingWithDetails(mapping) {
  try {
    const rolesData = await readJSON('roles.json');
    const menusData = await readJSON('menus.json');

    const roles = rolesData.roles || [];
    const menus = menusData || [];

    const role = roles.find((r) => r.id === mapping.roleId);
    const menu = menus.find((m) => m.id === mapping.menuId);
...
// GET /api/role-menu-mapping
router.get('/', authenticateToken, async (req, res) => {
  try {
    const data = await readJSON('roleMenuMappings.json');
    const mappings = data?.roleMenuMappings || [];

    const { id, roleId, menuId, includeDetails } = req.query;
    ...
    // Filter by menuId
    if (menuId) {
      filteredMappings = filteredMappings.filter((m) => m.menuId === menuId);
    }

    // Enrich with role and menu details if requested
    if (includeDetails === 'true') {
    ...
    console.error('Get role-menu mappings error:', error);
    res.status(500).json({ error: error.message || 'Failed to get role-menu mappings' });
  }
});

// POST /api/role-menu-mapping
router.post('/', authenticateToken, async (req, res) => {
  try {
    ...
    const { roleId, menuId, canView, canCreate, canUpdate, canDelete } = req.body;

    if (!roleId || !menuId) {
      return res.status(400).json({ error: 'Role ID and Menu ID are required' });
    }
    ...
    const existingMapping = mappings.find((m) =>
      m.roleId === roleId && m.menuId === menuId
    );
    if (existingMapping) {
      return res.status(400).json({
        error: 'Mapping already exists for this role and menu'
      });
    }
    ...
    const maxId = mappings.reduce((max, m) => {
      const num = parseInt(m.id.replace('rmm-', ''));
      return num > max ? num : max;
    }, 0);
    const newId = `rmm-${String(maxId + 1).padStart(3, '0')}`;
    ...
    await writeJSON('roleMenuMappings.json', { ...data, roleMenuMappings: mappings });

    const enrichedMapping = await enrichMappingWithDetails(newMapping);
    res.status(201).json({ mapping: enrichedMapping });
  } catch (error) {
    console.error('Create role-menu mapping error:', error);
    res.status(500).json({ error: error.message || 'Failed to create role-menu mapping' });
  }
});
...
router.put('/', authenticateToken, async (req, res) => {
  try {
    ...
    console.error('Update role-menu mapping error:', error);
    res.status(500).json({ error: error.message || 'Failed to update role-menu mapping' });
  }
});

// DELETE /api/role-menu-mapping
router.delete('/', authenticateToken, async (req, res) => {
  try {
    ...
    await writeJSON('roleMenuMappings.json', { ...data, roleMenuMappings: filteredMappings });

    res.json({
      message: 'Role-menu mapping deleted successfully',
      deletedMapping: mapping
    });
  } catch (error) {
    console.error('Delete role-menu mapping error:', error);
    res.status(500).json({ error: error.message || 'Failed to delete role-menu mapping' });
  }
});
